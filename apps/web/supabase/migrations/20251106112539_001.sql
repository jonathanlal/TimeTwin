...
-- Helper functions marked immutable keep generated columns timezone-safe.
create or replace function public.utc_trunc_minute(ts timestamptz)
returns timestamptz
language sql
immutable
as $$
...
create or replace function public.utc_hour_part(ts timestamptz)
returns smallint
language sql
immutable
as $$
  select extract(hour from ts at time zone 'UTC')::smallint;
$$;

create or replace function public.utc_minute_part(ts timestamptz)
returns smallint
language sql
immutable
as $$
  select extract(minute from ts at time zone 'UTC')::smallint;
$$;

create or replace function public.utc_hour_label(ts timestamptz)
returns text
language sql
immutable
as $$
  select lpad(extract(hour from ts at time zone 'UTC')::int::text, 2, '0')
       || ':' ||
         lpad(extract(hour from ts at time zone 'UTC')::int::text, 2, '0');
$$;
...
create table if not exists public.captures (
  ...
  minute_ts timestamptz generated always as (public.utc_trunc_minute(server_ts)) stored,
  label_idx smallint generated always as (public.utc_hour_part(server_ts)) stored,
  label_str text generated always as (public.utc_hour_label(server_ts)) stored,
  diff_seconds int not null check (diff_seconds between 0 and 59),
  source text check (source in ('app','widget','web')) default 'app',
  device_model text,
  app_version text,
  legacy_id text,
  check (public.utc_minute_part(server_ts) = public.utc_hour_part(server_ts))
);
